...
**Name collision, resolved (this was three-way as of this session):** (1) the "page 92" inbox drop, (2) crt's parked catalog split, (3) this new KB vision all claimed "bibliothecaire." Zach's call: the merged librarian project (2+3) owns the name; the page-92 idea is renamed **`quatre-vingt-douze.idea`** (`git mv` this session, content untouched — the rename candidate `fable-like/projects/bibliothecaire/README.md` already proposed). This is exactly the incident the queued NAMES.md item (fable-review, 2026-07-25, bottom of this file) exists to prevent — third arrival of the same noun makes that item's case for it.
