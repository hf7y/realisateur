...
- **2026-07-26 (nightly-batch, folded from `~/reports/realisateur/2026-07-26.md` PENDING by the same-day human-present migration session): three projects scaffolded overnight — informational flag.** `abletim` (`~/Documents/Projects/abletim`, dexter-pinned, parked on mandark — dexter-side enable is a human step), `bibliothecaire` (`~/Documents/Projects/bibliothecaire`, enabled at weight 1; NOTE its rename to `quatre-vingt-douze` is already queued as step 0 of the librarian drop — the name-collision judgment call from the same PENDING list was resolved by Zach in the 2026-07-26 `/ideate` session, merge + rename, so that half needs no answer), `secretaire` (`~/Documents/Projects/secretaire`, registered, parked `enabled=0`). All three live on `.scheduler/` from day one. This flag is closable by the next nightly once read.

- **2026-07-26 (nightly-batch, folded same as above): secretaire is registered but parked (`enabled=0`) — flip it on when you've answered its lane question.** Its remaining milestone items are human steps: fill `ACCOUNTS.md`'s inventory, and decide the access lane (grant-access vs utilities vs thunderbird — the fork is laid out in `secretaire/.scheduler/QUESTIONS.md`). Once a `> ` answer lands there, set `secretaire|1|...` in scheduler's `schedule/_paced.conf`.
  > (answer inline here)
