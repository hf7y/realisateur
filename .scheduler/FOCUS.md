...
**2026-07-29 (`/ideate`, Zach-directed — "any last blockers before the rest become data?"): YES. THE BINDING CONSTRAINT IS QUOTA, AND IT IS ON NO ONE'S LIST.** Six blockers found by probing rather than by re-reading our own records. Five are design blockers we already knew in some form. The first is physics and nobody has named it.

**B1 — NOTHING IS DISPATCHING ON EITHER HOST, AND DEXTER NEVER HAS.** Verified 2026-07-29 00:45 by counting log line-types on both hosts:
- **dexter: 0 DISPATCH, 0 RUN, 0 DONE — ever.** 463 HOLD across the whole log (~38h). Its rotation has `crt|1|3` and `wtul|1|1` enabled, and neither has ever been dispatched by the paced runner. The only prior evidence dexter works as an execution host is `crt`'s clone, whose last commit is **2026-07-25 20:34 — four days stale.**
- **mandark: last DISPATCH `2026-07-28T07:39:45` — 17 hours ago.** Dispatches/day: 07-24: **174**, 07-25: 29, 07-26: 39, 07-27: 63, 07-28: **5**. All five on 07-28 were `scheduler -> scheduler-dev-cycle.sh`, i.e. scheduler's own self-dev — the very thing last session's decision proposes to take dark.
- **Cause is the gate, and both hosts agree on the numbers:** `# HOLD -- 7d window 29% used vs burn-line 27% (on-pace, on-pace)`. Being marginally above the even-burn line is sufficient to hold. Interactive sessions draw on the same weekly quota, so a night of interactive work holds the automation down.
- Ceilings differ and it does not help: mandark `ceiling=0.99` (env override), dexter `ceiling=0.85` (default). The binding constraint is the burn-line, not the ceiling, so the override buys mandark nothing here.

**Why this is fatal to the play as designed rather than merely slow.** The play is an *alternating* rotation: scheduler's turn writes the next participant, that participant takes a turn, repeat. At the current dispatch rate the loop advances five times a day on mandark and zero times on dexter. **A self-extending rotation that cannot take a turn is not a slow migration, it is a stopped one** — and it will look healthy the whole time, because HOLD is a correct, on-pace verdict and not an error. ecosim's S2 (accumulation, T2) predicted declining useful-work-per-turn with no alarm; this is that, arriving before the sensor did.

**A NEAR-MISS WORTH RECORDING, because it is the session's own thesis.** The first probe reported "463 HOLD, 0 RUN" on dexter *and* "1360 HOLD, 0 RUN" on mandark, which reads as total system failure. It was wrong: `RUN` lines do not carry the `(gate` token my grep required, and mandark in fact has **320 RUN / 317 DISPATCH / 317 DONE**. A false catastrophe, produced by a pattern that could only match the HOLD path — **an output alphabet of `{HOLD}` in the probe, not in the system.** Caught by re-deriving before reporting. The corrected method (count line-types, do not grep for the outcome you expect) is what produced B1's real numbers.

**B2 — the freeze file does not exist.** M1(a) is the only surviving M1 mechanism and nothing implements it. Verified: no `FREEZE*` file in the scheduler repo, no commit mentioning one.
**B3 — `63cf3b4` is unanswered.** Both slots still read `> (answer inline here...)`. It is the precondition for the self-writing rotation, not an independent question (see `bde9e62` §2).
**B4 — scheduler is not a participant in `_paced.dexter.conf`.** It contains exactly `crt` and `wtul`. The bootstrap cannot take its own turn on dexter until it is in that file — and adding it is precisely the two-writer decision B3 gates.
**B5 — M1.5 does not exist, and 3 projects have no milestone to stash** (bibliothecaire, home-assistant, vkv-inventory). Filed `043de21`.
**B6 — `7fccdc1` q2 unanswered** (freeze scope over svc-vaporwave's fixed-cron jobs).
...
**2026-07-29 (`/ideate`, Zach-directed — "failures to file"): TWO FAILURES FROM TONIGHT, AND THEY ARE ONE CHAIN. Filed as BUILD-DISCIPLINE pattern 18.** Zach's words: *"we should have done the freeze before unlocking the usage gate. the usage gate was actually helpful and we missed it because it's called usage gate, not slow down metabolism."*

**FAILURE 1 — ordering. Mine; I ran the sequence.** The throttle came off (`e502555`) before the abort handle went on (`e92a493`, engaged `983ed3d`). The chain is short and entirely mechanical: gate removed → dexter dispatched within 5 minutes and looped on `crt` → **a busy host skips `PULL`** → the freeze, which propagates by git pull, could not reach the one host it most needed to reach → dexter had to be stopped by hand-editing its crontab. The freeze existed, was correct, was tested 17 ways in both directions, and was **irrelevant**, because the quiet window in which it could have been installed had already been spent unthrottling the system.
Note what this says about the freeze's own design: it is not merely that we ordered two steps wrongly. **A control that reaches a host only while that host is idle is not a control over a busy host** — and a busy host is the only case an abort handle exists for. The ordering error made a latent design flaw load-bearing on the same night.

**FAILURE 2 — the name, and this is the deeper one.** `usage-gate.sh` was read as a quota guard, because it is called a usage gate. That reading was locally correct: both hosts were stalled, the gate was holding at `29% used vs burn-line 27%`, credits were available, and removing the 7d even-burn hold worked exactly as intended. But it was not only a quota guard — it was the ecosystem's **metabolic rate limiter**, the single thing bounding how fast the whole system could consume itself, and it was doing that job well. **The name had fewer distinctions than the job**, so a true statement about the name licensed a false conclusion about the system.
This is the session's own thesis landing on us rather than on a script: a label is a sensor, and this one had one symbol where it needed two. Every other instance tonight was a mechanism that could not distinguish two world-states; this one was a *name* that could not, and the regulator it named was removed by people reasoning correctly about the wrong function.

**THE RULE (now pattern 18):** install and *verify arrival of* the replacement control before removing the existing one — arrival on every host, not merely a commit. A control you have not yet watched refuse something is not installed.
**THE TELL, generalised:** when a component's name describes its *input* (usage, disk, tokens, rate) rather than what breaks in its absence, assume the name understates it, and ask what it is the last line of defence for before touching it. **A control whose real function has no name is one nobody can argue for at the moment it is removed.**
...
**2026-07-29 (`/ideate`, Zach-directed): REALISATEUR IS THE SECOND PARTICIPANT IN THE PLAY, AND ITS TURN IS THE ECOSYSTEM REWEIGHT.** Zach, verbatim: *"no weights remove. realisateur comes back on next and assigns weights to the ecosystem."* Two decisions in one sentence.

**1. The stop verb removes weights; nothing is stashed.** The proposal filed minutes earlier (`scheduler 6877476`) called reversibility "the hard requirement" and specified a stash so a `resume` could restore prior weights. **Withdrawn** (`scheduler 9a4b4a1`). Weights are realisateur's to assign — already the standing division (`docs/priority-weight.md`; the weight field is this repo's own knob, explicitly distinct from scheduler's engine). So there is nothing to preserve.
**Why this is better than the stash rather than merely simpler:** a stashed weight is a *frozen judgment restored blind*. Tonight's weights encode a pre-migration ecosystem — `crt` at 3 on a host that had never dispatched, `bibliothecaire` raised to 2 hours ago for a research request, `scheduler` at 3 on the host it is being moved off. Restoring those after the migration would reinstate priorities chosen for a world that no longer exists, **and it would look like continuity rather than staleness** — the exact failure mode this repo has filed four times this week under a different name. Re-deriving is the correct operation and the organ that derives already exists.
Consequence for the seam: `stop` does **not** imply `resume`. It implies *stop, then a realisateur pass*. Scheduler owns dispatch mechanics; realisateur owns what deserves to run. The verb halts at its own boundary instead of reaching across it to restore someone else's judgment.

**2. Realisateur is next in the rotation.** The self-extending order now has two named links: scheduler bootstrapped itself (`5bde4d5`, authored "Dexter Pine", verified from mandark off the ref) and was armed on dexter (`f8609d0`); **realisateur follows, and its first act is assigning weights to the ecosystem.** This is the first time the play's participant order has been decided by anything other than "whoever is ready" — it is decided by *what the system needs next*, which is the readiness-over-round-robin principle the whole experiment exists to justify, arriving before the data that was supposed to justify it.

**What realisateur's turn actually owes, so it is not improvised at 3am:**
- A reweight of every registered project, derived fresh — not restored, not carried over. The pre-migration weights are explicitly not the baseline.
- It must state, per project, *why* — same standard as any weight change (`docs/priority-weight.md`), because a reweight with no rationale is indistinguishable from a restore.
- It runs on **dexter**, under the same bar scheduler met: a real run that commits and pushes from there. Anything less is not a migration turn.
- Its own conf still points at `/home/zach/git-remotes/realisateur.git`, one of the 11 confirmed unreachable-from-dexter. **Repointing its own `REPO_URL` is part of its bootstrap turn** — the payload-not-blocker decision (Zach 2026-07-29) applied to this repo specifically.

**OPEN, flagged not decided.** M1.5 (the milestone stash/restore) has the *identical* shape to the weight stash just withdrawn: amend a project's stability milestone to the bootstrap bar, restore after. If "do not stash, re-derive" is right for weights it may be right for milestones — **but a stability milestone is a project's own declared bar, not realisateur's judgment about it**, so the argument does not obviously transfer. Do not resolve this by analogy without asking.

  [batch] Realisateur's own bootstrap turn: repoint `REPO_URL` off the local bare to GitHub, then the ecosystem-wide reweight, derived fresh with per-project rationale. Runs on dexter; proof bar is commit-and-push from there.

**2026-07-29 (`/cloture`): SESSION CLOSE — THE BOOTSTRAP RAN, AND WHAT IS LEFT RUNNING UNATTENDED.** Zach went AFK with dexter live. This entry exists so the unattended window is legible to whoever reads next, including a run that cannot see the conversation.

**WHAT IS RUNNING WHILE NOBODY WATCHES (verified at close, not assumed):**
- **dexter**: cron `*/30 PACED_MAX_PER_TICK=1`, rotation `crt|0|0 wtul|0|0 scheduler|1|3`. Expect **~12 scheduler self-dev cycles over ~6h, ~37% duty cycle**. Each cycle merges to local main and **pushes to origin immediately** — that is by design (`DESIGN-NOTES.md`, push-on-cycle), and every merge prints its own revert handle (`git revert -m 1 <sha>`).
- **mandark**: cron `*/5` runner still ticking but **dispatches nothing** — all 9 enabled participants refused by `schedule/FREEZE`. Sweep backstop disabled. `weight-audit` fires 06:00.
- **weight-audit at 06:00 is predicted to be a no-op.** Dry-run at 03:06: `No park suggestions this run. No weight changes this run.` The darkened scheduler line is correctly invisible to it (0 scheduler blocks in output), so `58d6495` survives the 06:00 run. Prediction pre-registered with ecosim (#16 comment) *before* the event.
- **ecosim** is instrumenting, observer-only, no stop bit. Its `bin/migration-watch.py` holds rotation and milestone baselines captured before any override.

**THE RESULT.** Scheduler bootstrapped itself onto dexter and now works there unattended. Three merges tonight, all authored **"Dexter Pine"**, all verified from mandark off the ref rather than from dexter's own report: `5bde4d5`, `76e96b5`, and the cycle in flight at close. Its commits: `d62e961` *locate its own checkout, and refuse loudly when it cannot*; `46f51ae` *reconcile-witness reads the tree it ships in*; `a7066ff` *read THIS host's rotation, not always the shared one*.
**Read those three titles together.** Unprompted, across three consecutive cycles, the migrated agent attacked one defect class: **scheduler assuming it is the only host.** That is the migration's actual difficulty, chosen from inside the migration, by the thing being migrated. It is also the class that bit this session repeatedly — a stale mandark checkout showing `crt|1|3` against dexter's `0|0`, `sync-crontab.sh` not being host-scoped, and a bare `EXEMPT: scheduler` that unfroze both hosts at once.

**PHILOSOPHY DELTA — TWO NEW PATTERNS, both from Zach's own diagnoses:**
- **18 — a safeguard named for its mechanism gets removed by someone reasoning about its function.** `usage-gate.sh` read as a quota guard, because that is its name; it was also the ecosystem's metabolic rate limiter, and *that* function has no name, so nobody could argue for it when it was removed. Cost: the throttle came off before the abort handle went on.
- **19 — the operator reaches around the system instead of through it, and the bypass is invisible to the system.** Dexter was stopped by hand-edited crontabs and killed pids; scheduler's `run.log` shows a DISPATCH and no termination, and ecosim's sensors cannot see any of it.
Both are in `BUILD-DISCIPLINE.md` (`d19d75b`, `e6c1a87`), not just in chat.

**THE SESSION'S OWN BEST EVIDENCE FOR 18, recorded because it is uncomfortable and therefore worth keeping:** three separate probes I ran had false symbols. A `grep` for `RUN \(gate` that could only match HOLD, and nearly reported total system failure on both hosts. A `pgrep -f scheduler-dev-cycle.sh` that **matched its own command line**, causing me to report a finished cycle as running, twice. And an exit-0-with-no-output dry run I nearly accepted as proof. Every mechanism failure tonight was mirrored by a measurement failure, and the measurements were mine.
