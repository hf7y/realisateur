...
bin/tests/ci-provision.test.sh     realisateur follows its subject.
...
bin/selfdev-hooks-provision.sh     realisateur same claim and same row as bin/selfdev-permissions-provision.sh directly above -- the self-dev account CONTRACT, not the accounts themselves, and the same machine-wide-config notice to senechal applies. Filed as a sibling script rather than folded into that one because the two answer different questions (WHAT an account may do vs WHAT fires when a subagent stops) per #294's duplicated-truth argument, not because ownership differs.
bin/tests/selfdev-hooks-provision.test.sh realisateur follows its subject.
