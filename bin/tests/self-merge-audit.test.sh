#!/usr/bin/env bash
#
# TRAPS (see bin/self-merge-audit.sh's own header for the full reasoning):
# Cases, on fixture repos named after real roster entries so the positional
# filter (cli_require_matched) accepts them:
#   A protected repo, all merges self-merged   -> "ok", counted gated, no HAZARD
#   B unprotected repo, self-merged             -> HAZARD, counted, --strict exits 1
#   C protected repo, no self-merges at all      -> "ok", nothing counted
#   D repo unreadable                            -> BLIND, exit 6
#   E `gh pr list` itself fails                  -> BLIND, exit 6
#   F repo with no merged PRs at all              -> "ok", nothing counted
#   G --verbose lists every self-merge, gated or not; bare invocation does not
#   H bare invocation exits 0 despite a HAZARD; --strict exits 1
#
# Usage: bin/tests/self-merge-audit.test.sh   (exit 0 = all pass)
...
# The stub speaks the two API shapes the script uses: `gh api repos/...` and
# `gh pr list --repo ... --json number,author,mergedBy`.
#   realisateur -- protected, two PRs, both self-merged by the same account
#   chezz       -- no required check, one self-merged PR -> HAZARD
#   crt         -- protected, PRs merged by a DIFFERENT account -> no self-merge
#   ecosim      -- protected, `gh pr list` returns an empty array
#   basheur     -- readable repo, but `gh pr list` itself fails (BLIND)
#   gardien     -- repo unreadable at all via `gh api` (BLIND)
cat > "$T/gh" <<'EOF'
#!/usr/bin/env bash
if [ "$1" = "pr" ] && [ "$2" = "list" ]; then
  repo="" prev=""
...
# --- A: protected + self-merged -> gated, never a HAZARD ---------------------
out="$(run realisateur)"
...
# --- B: unprotected + self-merged -> HAZARD -----------------------------------
out="$(run chezz)"
...
# --- C: protected, but merger != author -> no self-merge at all --------------
out="$(run crt)"
...
# --- E: repo readable, but `gh pr list` fails -> BLIND ------------------------
out="$(run basheur)"
...
# --- G: --verbose lists every self-merge; bare invocation does not -----------
out="$(run realisateur)"
...
# --- H: bare invocation never gates on a HAZARD; --strict does ---------------
...
# --- roster filter: an unknown repo name is a usage error, not a silent skip -
