...
if [ "$(cat "$T/hj/acctj/.claude/hooks/pretooluse-memory-budget.sh")" = "$(cat "$SRC")" ]; then
  ok "J: --apply installs the PreToolUse memory-budget hook file too, absent -> present (#715)"
else
  bad "J: --apply did not install pretooluse-memory-budget.sh"
fi
if [ "$(cat "$T/hj/acctj/.claude/hooks/session-start-memory-budget.sh")" = "$(cat "$SRC")" ]; then
  ok "J: --apply installs the SessionStart memory-budget hook file too, absent -> present (#715)"
else
