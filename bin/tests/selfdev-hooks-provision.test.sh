...
has "I: Stop is the wired event too (#681)" "$WANT" "Stop"
has "I: the residue-gate hook is the command" "$WANT" "stop-residue-gate.sh"
...
[ "$(printf '%s' "$WANT" | jq -r '.Stop[0].hooks[0].type')" = "command" ] \
...
run_hj() { HOME_ROOT="$T/hj" SUDO='' SELFDEV_HOOK_SRC="$SRC" SELFDEV_STOP_HOOK_SRC="$SRC" SELFDEV_SESSIONSTART_HOOK_SRC="$SRC" SELFDEV_PRETOOLUSE_HOOK_SRC="$SRC" SELFDEV_CREDENTIAL_HOLD_HOOK_SRC="$SRC" "$SCRIPT" "$@" 2>&1; }
...
if [ "$(cat "$T/hj/acctj/.claude/hooks/stop-residue-gate.sh")" = "$(cat "$SRC")" ]; then
  ok "J: --apply installs the Stop hook file too, absent -> present"
else
  bad "J: --apply did not install stop-residue-gate.sh"
fi
if [ "$(cat "$T/hj/acctj/.claude/hooks/session-start-verb-pin.sh")" = "$(cat "$SRC")" ]; then
  ok "J: --apply installs the SessionStart hook file too, absent -> present (#708)"
else
  bad "J: --apply did not install session-start-verb-pin.sh"
fi
if [ "$(cat "$T/hj/acctj/.claude/hooks/pretooluse-path-guard.sh")" = "$(cat "$SRC")" ]; then
  ok "J: --apply installs the PreToolUse hook file too, absent -> present (#707)"
else
  bad "J: --apply did not install pretooluse-path-guard.sh"
fi
if [ "$(cat "$T/hj/acctj/.claude/hooks/pretooluse-credential-hold.sh")" = "$(cat "$SRC")" ]; then
  ok "J: --apply installs the credential-hold hook file too, absent -> present (#714)"
else
  bad "J: --apply did not install pretooluse-credential-hold.sh"
fi

O="$(run_hj)"
case "$O" in *"hook FILE is"*) bad "J: a current hook file should not report drift: $O" ;;
  *) ok "J: all five current hook files stop being a finding" ;; esac

O="$(HOME_ROOT="$T/hj" SUDO='' SELFDEV_HOOK_SRC="$T/no-such-build" SELFDEV_STOP_HOOK_SRC="$T/no-such-build" SELFDEV_SESSIONSTART_HOOK_SRC="$T/no-such-build" SELFDEV_PRETOOLUSE_HOOK_SRC="$T/no-such-build" SELFDEV_CREDENTIAL_HOLD_HOOK_SRC="$T/no-such-build" "$SCRIPT" 2>&1)"
