...
if want arming; then
  ...
  if ! printf '%s' "$st" | jq -e '.accounts' >/dev/null 2>&1; then
    ...
    # TRAP (#919): fromdateiso8601 accepts only a "Z" suffix. This used to
    # patch that with sub("\\+00:00$"; "Z"), which normalised the one offset
    # the collector never emits while missing every offset it does (the live
    # document carries a local, often negative, offset). Fixed at the source
    # instead: monkey-status-collect.py now emits started_at/ended_at as UTC
    # Z, so the consumer reads a document with one timestamp format, not two,
    # and does not need to patch offsets here. A jq failure is BLIND, never
    # clean, if some other field still slips one through.
