...
if want arming; then
  ...
  if ! printf '%s' "$st" | jq -e '.accounts' >/dev/null 2>&1; then
    ...
    # fromdateiso8601 only reads "Z"; jq has no timezone arithmetic to rebase
    # any other offset onto it, so the collector (to_utc_z, #919) normalises
    # started_at/ended_at to "Z" before publishing -- never another offset here.
