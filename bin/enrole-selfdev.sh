...
if [ "$SYNC" -eq 1 ] && [ "$MODE" != --check ] && [ -n "$ON" ]; then  # driven from here over ssh (realisateur#895); dose is already a verb on the target, so this uses the transport's own knobs directly rather than its ship_run helper -- the same remote sudo -u shape this repo's other remote-apply steps use
  ...
  if [ "${#RUN[@]}" -gt 0 ]; then
    out="$("${RUN[@]}" "dose '$PROJECT' --apply" 2>&1)"; rc=$?
    printf '%s\n' "$out" | sed 's/^/    /'
    if [ "$rc" -ne 0 ]; then bad "dose $PROJECT --apply exited $rc"
    elif printf '%s' "$out" | grep -qE '^(BROKEN|BLIND|GAP):'; then bad "dose $PROJECT --apply printed BROKEN/BLIND/GAP lines -- read them above"
    else ok "crontab converged for $PROJECT (dose $PROJECT --apply)"; fi
  fi
elif [ "$SYNC" -eq 1 ]; then
