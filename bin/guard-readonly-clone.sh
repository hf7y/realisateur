...
CLI_SUMMARY="refuse a local commit into a self-dev account's foreign clone of scheduler/realisateur/senechal -- NOTHING GATES THE PUSH (corrected 2026-09, #1136): an https push authenticates as whatever this account's git credential helper is -- the shared token, or, since the App migration, a per-account GitHub App installation token -- and neither is read-only. wire-selfdev-git.sh's deploy keys were never that barrier either: they are read-only over ssh, but no url.insteadOf has rewritten an https remote onto that ssh alias since #171, so they go unread by every push and fetch this ecosystem actually performs. This hook is commit-time and path-based instead: it is the only thing that stopped wtul's scheduler dispatch clone from diverging and dispatching stale code for 3 ticks before a human reset it by hand (hf7y/scheduler#321), and it stops the local commit landing, never a push -- a foreign clone with an existing bad commit, or one made outside this hook's reach, can still push freely; that would need a pre-push hook, which does not exist. Path-based, evaluated live at commit time: only a clone directly under \$PROJECTS/<name> where <name> != this account is refused; a scratch clone, a worktree, or anywhere else (CLAUDE.md rule 6's cross-repo draft-PR workflow) is untouched. Shares stamp-verb-build.sh's HOOK_DIR: one core.hooksPath slot, two independently-generated files (prepare-commit-msg, pre-commit)."
