...
# GATE: none -- reads live label lists; writes only with --apply
#
# hf7y/scheduler#318: this used to also reconcile a `needs-human` label
# against the DECISION:/NO-DECISION: line-1 sentence in every open issue's
# body. That reading is GONE -- GitHub's own `assignees` field is the
# "waiting on a human" signal now (tempo.sh already reads it directly), and
# this file no longer derives or writes anything to replace it. What is left
# is the other half it always did: does a repo carry the labels the grammar
# declares at all.
set -uo pipefail

CLI_NAME='etiquette'
CLI_SUMMARY='the estate label grammar, and whether a repo carries it'
CLI_USAGE='  etiquette                        print the grammar every repo follows
  ...
  etiquette <owner>/<repo> --apply provision the labels the grammar declares
  etiquette --all [--apply]        the same, over every repo in bin/lib/roster-set.sh'
CLI_FLAGS='--all --apply --path'
CLI_POSITIONAL='[<owner>/<repo>]'
CLI_EXITS='  0  the repo carries every label the grammar declares
  1  findings: a declared label is missing from the repo
  2  usage error
  6  BLIND -- the grammar or the label list could not be read. Never 0.
...
if [ -z "$REPO" ]; then
  ...
  say ""
  say "SOURCE column: written:<verb> belongs to that verb alone; typed is a"
  say "person's or agent's own judgement. etiquette derives none of it -- it only"
  say "checks that a repo carries the label, whatever writes it."
  exit 0
fi

# --- does the repo carry the declared labels? -------------------------
...
say "$label_findings label finding(s); $provisioned label(s) provisioned."
[ "$label_findings" -gt 0 ] && [ "$APPLY" -eq 0 ] && say 'Re-run with --apply to create the missing label(s).'
[ "$label_findings" -eq 0 ] || exit 1
exit 0
