...
"cost|hf7y/etalon prose guard -- a PR fails if >30%% of added lines are markdown, or it adds a top-level .md|the /nightly-batch ideation brief, which asked runs to infer ideas and scaffold projects|doctrine filed as engineering tickets (7 closed into the vault stream)|test -f .github/workflows/prose.yml"
"homes|bin/hardcoded-home-lint.sh -- refuses an absolute path into a named user's home, in code|/home/zach hardcoded in five retired bashify libs|verb-set.test.sh running only on the one machine where that path existed|test -x bin/hardcoded-home-lint.sh"
...
"sonde|a wrapper speaking sonde's exit vocabulary (0/8/9/6, not 0/1/2/3), now ecosim's own bin/ecosim-sensor-tick.sh (#49 moved it out of this repo entirely)|the ecosim-sensor path that no build ever contained|exit code 3, which means BLIND upstream and needs-summon here|! test -e bin/ecosim-sensor-tick.sh"
