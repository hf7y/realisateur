...
#
# Zach-directed, 2026-08-04. `zach@monkey` has sudo but not NOPASSWD, which
# makes every privileged provisioning step on that host require a human at a
...
# script touches ONLY `zach`, the hands account that already has sudo. The
# project users are untouched and must stay that way.
#
# WHAT IT DOES COST, stated rather than buried: after this, anything able to
# execute as zach@monkey has root on monkey with no further check -- including
# anyone holding `selfdev_monkey`, which lives on dexter. That is a real
# widening. It is recorded here so the next reader sees the trade rather than
# inferring that nobody thought about it.
#
#   ./monkey-nopasswd.sh --check      is it already in place? no privilege needed
#   ./monkey-nopasswd.sh --install    install it (prompts for the password ONCE)
#
# --install needs a TTY, because the whole point is that it asks you for the
# password one final time. Run it from a terminal, not from an unattended job.
#
# SAFETY. A malformed file in /etc/sudoers.d can break sudo for every user on
# the host, and the recovery for that on a headless VM is unpleasant. So the
# candidate is written to a temp file, validated with `visudo -c` FIRST, and
# only installed if validation passes. It is never edited in place.
